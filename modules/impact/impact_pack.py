...
class FaceDetailer:
    ...
    def doit(self, image, model, clip, vae, guide_size, guide_size_for, max_size, seed, steps, cfg, sampler_name, scheduler,
             ...
             bbox_threshold, bbox_dilation, bbox_crop_factor,
             sam_detection_hint, sam_dilation, sam_threshold, sam_bbox_expansion, sam_mask_hint_threshold,
             sam_mask_hint_use_negative, drop_size, bbox_detector, wildcard, cycle=1, enabled=True,
        ...
        # If disabled, return input image with empty outputs
        if not enabled:
            empty_cropped = [empty_pil_tensor()]
            # Create empty mask with correct dimensions matching the input image
            empty_mask = torch.zeros((image.shape[0], image.shape[1], image.shape[2]), dtype=torch.float32, device="cpu")
            pipe = (model, clip, vae, positive, negative, wildcard, bbox_detector, segm_detector_opt, sam_model_opt, detailer_hook, None, None, None, None)
            return image, empty_cropped, empty_cropped, empty_mask, pipe, empty_cropped
        ...
        if len(image) > 1:
            print(f"[Impact Pack] WARN: FaceDetailer is not a node designed f or video detailing. If you intend to perform video detailing, please use Detailer For AnimateDiff.")
